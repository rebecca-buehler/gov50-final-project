...
In my final visualization, I plan to superimpose a graph of fatalities from events that targeted women specifically over this graph of all civilian fatalities. 
...
```{r}
Ethiopia_1997_2023_Sep22 <- read_excel("~/Documents/Thesis/Data/ACLED/Ethiopia_1997-2023_Sep22.xlsx") 

civilian_violence <- Ethiopia_1997_2023_Sep22 |>
  filter(CIVILIAN_TARGETING == "Civilian targeting", EVENT_DATE >= "2020-11-03") 
sum(civilian_violence$FATALITIES)

women_violence <- Ethiopia_1997_2023_Sep22 |>
...
civilian_violence_plot <- civilian_violence |>
  ...
  geom_col(mapping = aes(x = month, y = fatalities_per_month), 
           fill = "darkslateblue") +
...
civilian_violence_plot <- women_violence |>
  ...
  geom_col(mapping = aes(x = month, y = fatalities_per_month), 
           fill = "darkslateblue") +
  geom_col(mapping = aes(x = month, y = women_fatalities_per_month), 
           fill = "pink") +
  theme(axis.text.x = element_blank(), 
        panel.spacing = unit(0.05, "lines"),
        strip.placement = "outside") +
